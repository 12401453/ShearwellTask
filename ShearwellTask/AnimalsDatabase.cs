...
namespace ShearwellTask
{
    ...
    public class AnimalsDatabase
    {
        ...
        public async Task<string> GetGroupNamesByGroupFlagAsync(GroupFlags group_flag)
        {
            if (db_conn == null) await InitAsync();
            try
            {
            ...
            {
                StatusMessage = string.Format("Failed to retrieve group in GetGroupNamesByGroupFlag() call with error-message: {0}", ex.Message);
        ...
        public async Task InsertNewAnimalAsync(Animal new_animal)
        {
            int result_code = 0;
            if (db_conn == null) await InitAsync();
            try
